...
" Order By PnStZapisa desc ", con);
 ...
 + "select top  " + Top2 + "  PnStZapisa,DATEDIFF(MINUTE,PnDatOdh, PnDatPrih) AS Duration from PotNal  where PnDelavec = " + Radnik + " and PnZnesOrg = 3471 and PnStatus = 'OD'  Order By PnStZapisa desc) t1 "
...
+ "select top  " + Top2 + "  PnStZapisa,DATEDIFF(MINUTE,PnDatOdh, PnDatPrih) AS 'Duration' from PotNal  where PnDelavec = " + Radnik + " and PnZnesOrg = 1735.5 and PnStatus = 'OD'  Order By PnStZapisa desc) t1 "
   ...
   "   ,[RedovanRadIZ]      ,[GodOSati]       ,[GodIZNOS]      ,[BOL100Sati]      ,[BOL100IZNOS]      ,[BOL65Sati] "+
    ...
    " where Zarada.Fiksna = 0  ";
      ...
      FROM [Perftech_Beograd].[dbo].[ObracunZaposleni]
            ...
            dataGridView1.Columns[10].HeaderText = "Pocetna";
        ...
        private void PovuciBolovanje65()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                   ...
                   // Select Cast((isnull(Sum(Ukupno),0)) as integer) as UK from Bolovanje where ZaposleniID = " + row.Cells[0].Value +
                     //   " and Convert(nvarchar(10), DatumOd, 126) >= '" + dtpVremeDo2.Value.ToString("yyyy-MM-dd") + "' and Convert(nvarchar(10), DatumDo, 126) <= '" + dtpVremeDo2.Value.ToString("yyyy-MM-dd") + "' and TipBolovanja = '65'", con);
