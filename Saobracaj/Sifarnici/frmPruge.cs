...
namespace Saobracaj.Sifarnici
{
    public partial class frmPruge : Form
    {
        ...
        int ID,Elekt;
        string Oznaka, Naziv;
        public frmPruge(int id,string oznaka,string naziv,int elekt)
        {
            InitializeComponent();
            ID = id;
            Elekt = elekt;
        ...
        private void VratiStanice()
        {
            ...
            txtOznaka.Text= Oznaka.ToString();
            txtOpis.Text = Naziv;
            if (Elekt == 1)
            {
                chkElektrificirana.Checked = true;
            }
            else
            {
                chkElektrificirana.Checked = false;
            }

            var select = "select PrugaStavke.ID,RB,IDPruge,stanice.Opis,stanice.Kod as UIC, StacionazaKM,MNOU,MNOP,Klasa,Osovinsko " +
                "From PrugaStavke " +
                "inner join stanice on PrugaStavke.StanicaOD = stanice.ID Where IDPruge =" + ID + " order by RB asc";
